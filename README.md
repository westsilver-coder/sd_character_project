...
- **기본 동작**: 입력 사진은 **특징 추출만** 합니다. 머리색·피부톤·의상·머리 길이 등을 텍스트로 뽑아서 **프롬프트**에 넣고, 2D 생성은 **txt2img만** 사용합니다.  
  → **실사 이미지를 그대로 변형하지 않고**, 추출한 특징으로 **새 캐릭터**를 그립니다.
- **`--img2img`** 옵션을 주면, 입력 사진을 Img2Img의 초기 이미지로 써서 닮은 캐릭터를 만들 수 있습니다. (실사가 약간 녹아들 수 있음.)
- **회전 이슈**: 스마트폰 등에서 찍은 사진은 EXIF 방향 정보가 있어서, 코드에서 **EXIF를 적용**해 세로/가로를 보정한 뒤 특징 추출과(선택 시) Img2Img를 사용합니다.
...
- 기본: 입력 사진은 **특징 추출만**, 2D는 **txt2img** (실사 변형 없음)
- `--img2img` : 입력 사진을 Img2Img 초기 이미지로 사용
- `--strength F` : `--img2img` 사용 시 strength. 기본 0.55
